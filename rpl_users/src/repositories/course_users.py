...
class CourseUsersRepository(BaseRepository):
    ...
    def exists_course_by_name_subject_id_semester_and_admin(self, name: str, subject_id: str, semester: str, admin_id: int) -> bool:
        return (
            self.db_session.execute(
                sa.select(Course).where(
                    ...
                    Course.course_users.any(
                        CourseUser.user_id == admin_id
                    ),
                )
            )
            .scalars()
    ...
    def create_course_user(self, course_id: int, user_id: int, role_id: int, accepted: bool) -> CourseUser:
