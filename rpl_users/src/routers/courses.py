...
@router.get("/courses", response_model=List[CourseWithUserInformationResponseDTO])
...
def get_all_course_users_from_course(
    ...
    return CoursesService(db).get_all_course_users_from_course(course_id, current_user, role_name, student_id, return_profile_pictures)
...
def get_all_roles(db: DBSessionDependency):
...
def get_all_universities(db: DBSessionDependency):
...
def course_user_auth_from_activities_api(
    course_id: int, current_user: CurrentUserDependency, db: DBSessionDependency
):
