...
@router.post(
    "/courses/{course_id}/activities", response_model=ActivityResponseDTO, status_code=status.HTTP_201_CREATED
)
def create_activity(
    course_id: int,
    current_course_user: CurrentCourseUserDependency,
...
def update_activity(
    ...
    return ActivitiesService(db).update_activity(
        current_course_user, course_id, activity_id, new_activity_data
    )
